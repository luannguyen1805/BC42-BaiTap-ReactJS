...
function ProductManagement() {
    ...
    const fetchProducts = async () => {
        try{
            const reponse = await axios.get("https://63e86446ac3920ad5beb7d4d.mockapi.io/api/products", 
            {params:{
    ...
    const handeleSubmit = async (product, type) => {
        ...
        try{
            if(id){
                //Cập nhật
                await axios.put(`https://63e86446ac3920ad5beb7d4d.mockapi.io/api/products/${id}`, payload);
            }else{
                // Thêm mới
                await axios.post("https://63e86446ac3920ad5beb7d4d.mockapi.io/api/products", payload);
            }
        }catch (error){
            console.log(error);
        }
    ...
    const handleDeleteProduct = async (productId) => {
        try{
            await axios.delete(`https://63e86446ac3920ad5beb7d4d.mockapi.io/api/products/${productId}`);
            fetchProducts();
        }catch(error){
            console.log(error);
        }
